...
#include <iostream>
#include<algorithm>
objectCodeMap::objectCodeMap()
{
    objectCodeMap::setTheMap();
}

std::unordered_map<std::string, Ob*> objectCodeMap::getObjectCodes(){
    return ObjectCodes;
}

int objectCodeMap::getTheCode(std::string ins){
...
int objectCodeMap::getTheFormat(std::string ins){
    transform(ins.begin() , ins.end() ,ins.begin() , ::toupper);
    if (ins[0] == '+'){
        ins.replace(0,1,"");
        return 4;
...
int objectCodeMap::getet(){
    return 4;
}

void objectCodeMap::setTheMap(){
